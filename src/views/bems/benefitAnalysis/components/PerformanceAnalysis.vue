...
<template>
  ...
  <div class="point-data-statistics">
    ...
    <div class="right-content">
      <div class="chart-container">
        <a-radio-group v-model:value="dateType" button-style="solid" @change="ragioChange">
          <a-radio-button value="date">日成本</a-radio-button>
          <a-radio-button value="month">月成本</a-radio-button>
          <a-radio-button value="year">年成本</a-radio-button>
        </a-radio-group>
        日期：<a-date-picker v-model:value="date" :picker="dateType" valueFormat="YYYY-MM-DD" />
        &nbsp;
        <a-button type="primary" @click="findData">查询</a-button>
      ...
      <!-- 图表下方的表格 -->
      <div class="table-container">
        <a-table :columns="tableColumns" :data-source="tableData" :pagination="false" bordered tableLayout="fixed" :scroll="{ x: 1500, y: 300 }" />
      </div>
    </div>
  </div>
</template>
