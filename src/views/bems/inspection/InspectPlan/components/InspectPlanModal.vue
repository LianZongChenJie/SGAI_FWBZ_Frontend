      ...
      <template #frequencyDisplay>
        <div class="frequency-display" v-if="formState.disposable === true">
          <div class="frequency-text">
            每 {{ formState.frequency }}{{ timeTypeText }} 的
          </div>
          <div class="frequency-tags">
            <a-tag 
              v-for="(tag, index) in formState.specificTime" 
              :key="index" 
              color="blue"
            >
  ...
  updateSchema([
    ...
    },
    {
      ...
      componentProps: {
        ...
        treeCheckable: true,
        multiple: true,
        onChange: (value: string[] = [], labelList: string[] = []) => {
...
async function handleSubmit() {
  try {
    await setFieldsValue({
      ...
      frequency:
        formState.frequency === '' || formState.frequency == null
          ? undefined
          : Number(formState.frequency),
      specificTime: normalizeSpecificTime(formState.specificTime).length
        ? normalizeSpecificTime(formState.specificTime)
        : undefined,
