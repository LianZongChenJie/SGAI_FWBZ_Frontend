...
import { getPlanControlDetailApi, getNotConfiguredListApi, spaceTree, categoryTree, savePlanControlRelApi } from '../Standardized.api';
...
const showModal = async (record) => {
  ...
  title.value = `配置：${record.spaceName} | ${record.planName}`
...
const getConfiguredTableData = async () => {
  let res = await getPlanControlDetailApi({ id: formState.value.id });
  if(res.circuits === null) res.circuits = []
...
const onSubmit = async () => {
  let params = {
    planControlId: formState.value.id,
    circuitIds: selectedIds.value,
  }
  let res = await savePlanControlRelApi(params)
