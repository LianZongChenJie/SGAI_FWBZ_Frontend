<template>
  <BasicModal v-bind="$attrs" @register="registerModal" destroyOnClose :width="800" :title="getTitle" @ok="handleSubmit">
    <BasicForm @register="registerForm" name="EnergyMediumManageForm" />
  </BasicModal>
</template>
<script lang="ts" setup>
  ...
  import { formSchema } from '../EnergyMediumManage.data';
  import { loadTreeData, saveOrUpdateDict } from '../EnergyMediumManage.api';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    // 更新标准单位选项，使用 props 中的 units
    const unitOptions = new Array<any>();

    if (data.unitData && data.unitData.length > 0) {
      for (let i = 0; i < data.unitData.length; i++) {
        unitOptions.push({
          label: data.unitData[i].name,
          value: data.unitData[i].id,
        });
      }
    }
