...
import { getSituationalDetailApi, getNotConfiguredListApi, spaceTree, categoryTree, saveScenariosRelApi } from '../Standardized.api';
...
const showModal = async (record) => {
  ...
  title.value = `配置：${record.spaceName} | ${record.groupName}`
...
const getNotConfigTableData = async () => {
  let params = {
    name: formState.value.name ? formState.value.name : undefined,
    num: formState.value.num ? formState.value.num : undefined,
  };
...
const getConfiguredTableData = async () => {
  let res = await getSituationalDetailApi({ id: formState.value.id });
  configuredDataSource.value = [...res.circuits];
...
const rightTableData = computed(() => {
  console.log('rightTableData------------->', selectedIds.value);
...
const onSubmit = async () => {
  let params = {
    scenariosId: formState.value.id,
    circuitIds: selectedIds.value,
  }
  let res = await saveScenariosRelApi(params)
