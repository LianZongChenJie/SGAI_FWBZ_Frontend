<template>
  <div class="page-main maintenance-plan-mangent-main-box" v-loading="loading">
    <el-container>
      <el-main>
        <div class="search-box clearfix">
          <el-form class="form-main" ref="searchFormRef" :model="searchForm" inline label-position="right"
            label-width="100px">
            <el-form-item label="时间：">
              <el-date-picker v-model="searchForm.planBeginTime" format="YYYY-MM-DD HH:mm:ss"
                value-format="YYYY-MM-DD HH:mm:ss" type="datetime" placeholder="时间">
              </el-date-picker>
            </el-form-item>
        ...
        <el-table :data="planList" tooltip-effect="dark" border @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="name" label="计划名称" align="center" show-overflow-tooltip
            min-width="140"></el-table-column>
          <!--          <el-table-column prop="taskTypeName" label="计划类型" align="center" show-overflow-tooltip></el-table-column>-->
          <el-table-column prop="createTime" label="创建时间" align="center" width="160"></el-table-column>
          <el-table-column prop="weibaoType" label="维保类型" align="center" show-overflow-tooltip
          ...
          <el-table-column label="操作" align="center" width="120" fixed="right">
            <template v-slot="scope">
              <a-popconfirm title="确认即时生效？" ok-text="确定" cancel-text="取消" @confirm="toEffective(scope.row)">
                <el-button type="text" v-if="scope.row.planState === '待派发'"
                  v-permission="'plan-effective'">即时生效</el-button>
              </a-popconfirm>
              <el-button type="text" v-if="scope.row.planState === '待派发'" @click="showEditDialog(scope.row)"
                v-permission="'plan-edit'">修改</el-button>
        ...
        <div class="pagination-box">
          <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" background
            :current-page="page" :page-sizes="[10, 20, 50]" :page-size="pagesize" :total="total"
            layout="total,prev, pager, next, sizes,jumper" v-show="planList.length"></el-pagination>
        </div>
      </el-main>
    </el-container>
    <!-- 计划修改弹出框 -->
    <el-dialog :close-on-click-modal="false" title="设备维保计划修改" v-model="editDialogVisible" width="960px"
      :before-close="closePlanDialog">
        ...
        <el-row :gutter="30" class="el-form-row">
          <el-col :span="12">
            <el-form-item label="执行科组：" prop="principalGroup">
              <el-cascader ref="" placeholder="" v-model="managementGroup" :options="groupOptions" :props="props"
                :clearable="false" style="width:100%" :show-all-levels="false" @change="getOrg"></el-cascader>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="30" class="el-form-row">
          <el-col :span="12">
            <el-form-item label="开始时间：" prop="planBeginTime">
              <el-date-picker type="date" v-model="planForm.planBeginTime" value-format="YYYY-MM-DD HH:mm:ss"
                format="YYYY-MM-DD HH:mm:ss" :picker-options="startPickerOptions"></el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="结束时间：" prop="planEndTime">
              <el-date-picker type="date" v-model="planForm.planEndTime" value-format="YYYY-MM-DD HH:mm:ss"
                format="YYYY-MM-DD HH:mm:ss" :picker-options="endPickerOptions"></el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="30" class="el-form-row">
    ...
    <!-- 计划详情弹出框 -->
    <el-dialog :close-on-click-modal="false" class="dialog-box" title="设备维保计划详情" v-model="detailDialogVisible"
      width="1300px">
            ...
            <el-container>
              ...
              <el-container v-if="activeIndex == 0">
                <el-main>
                  ...
                  <el-descriptions title="" :column="2" border :labelStyle="{
                    width: '20%',
                  }" :contentStyle="{
                    width: '100px',
                    overflow: 'hidden',
                    ...
                    <el-descriptions-item label="任务计划ID">{{
                      planDetail.id
                    }}</el-descriptions-item>
                    <el-descriptions-item label="维保计划名称">{{
                      planDetail.name
                    }}</el-descriptions-item>
                    <el-descriptions-item label="维保类型">{{
                      planDetail.weibaoType
                    }}</el-descriptions-item>
                    <el-descriptions-item label="执行人">{{
                      planDetail.executor
                    }}</el-descriptions-item>
                    <el-descriptions-item label="执行科组">{{
                      planDetail.principalGroup
                    }}</el-descriptions-item>
                    ...
                    <el-descriptions-item label="计划开始时间">{{
                      planDetail.planBeginTime.split(' ')[0]
                    }}</el-descriptions-item>
                    <el-descriptions-item label="计划结束时间">{{
                      planDetail.planEndTime.split(' ')[0]
                    }}</el-descriptions-item>
                    <el-descriptions-item label="实际开始时间">{{
                      planDetail.realBeginTime
                    }}</el-descriptions-item>
                    <el-descriptions-item label="实际结束时间">{{
                      planDetail.realEndTime
                    }}</el-descriptions-item>
                    <el-descriptions-item label="实际响应时间">{{
                      planDetail.realResponseTime
                    }}</el-descriptions-item>
...
const querySearch = (queryString, cb) => {
  ...
  var results = queryString
    ? userOptions.filter(this.createFilter(queryString))
    : userOptions
  cb(results)
}
...
async function showEditDialog(row) {
  ...
  if (row.departmentId) {
    ...
    managementGroup.value = getAllParentArr(
      groupOptions.value,
      row.departmentId
    )
...
async function showDetailDialog(row) {
  ...
  if (res) {
    ...
    recordList.value = res.operateRecordList
      ? res.operateRecordList
      : []
    ...
    maintenanceList.value = res.workOrderList
      ? res.workOrderList
      : []
  }
}
