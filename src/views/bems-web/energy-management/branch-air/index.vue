<template>
  <div class="energy-page">
    ...
    <section class="system-metrics">
      <article v-for="metric in distributedMetrics" :key="metric.key" class="metric-card" :class="metric.tone">
    ...
    <main class="dashboard-body">
      <section class="content-grid">
        <article class="panel topology-panel">
          ...
          <div class="topology">
            <DistributedAirCooledSchematic :values="values" @select-device="openDevice" />
        ...
        <aside class="side-stack">
          <article class="panel health-panel">
            ...
            <div class="donut-row">
              <div class="donut"><div><strong>85</strong><span>健康度</span></div></div>
              <div class="health-list"><span><i class="ok"></i>运行设备 <b>15</b></span><span><i class="idle"></i>停止设备 <b>6</b></span><span><i class="warn"></i>活动告警 <b>{{ point('distributed.station.alarmCount') }}</b></span></div>
            </div>
          </article>

          <article class="panel detail-panel">
            <div class="compact-heading"><div><span class="eyebrow">SYSTEM OVERVIEW</span><h3>分馆能耗对比</h3></div></div>
            <div class="detail-grid">
              <div><span>东会议室功率</span><strong>{{ display('distributed.east.loop.flow') }}</strong></div>
              <div><span>2号馆功率</span><strong>{{ display('distributed.hall2.loop.flow') }}</strong></div>
              <div><span>3号馆功率</span><strong>{{ display('distributed.hall3.loop.flow') }}</strong></div>
              <div><span>综合 COP</span><strong>{{ display('distributed.station.cop') }}</strong></div>
            </div>
          ...
          <article class="panel alarm-panel">
            ...
            <div class="alarm-list">
              <div><i class="level medium"></i><span><b>2号馆系统为手动模式</b><small>运行提示 · 10:00:00</small></span><em>中</em></div>
              <div class="normal"><i class="level"></i><span><b>东会议室1#机组正常启动</b><small>运行记录 · 09:35:12</small></span><em>正常</em></div>
              <div class="normal"><i class="level"></i><span><b>3号馆补水箱液位正常</b><small>运行记录 · 09:20:00</small></span><em>正常</em></div>
            </div>
          </article>
        </aside>
      </section>
...
<style scoped>
.energy-page { --ink:#2d3748; --muted:#718096; --edge:#e2e8f0; --panel-bg:#ffffff; --cyan:#0ea5e9; --blue:#3b82f6; --green:#22c55e; min-width:0; min-height:0; height:100%; color:var(--ink); background:#f5f7fa; display:flex; flex-direction:column; overflow:hidden; font-family:"PingFang SC","Microsoft YaHei",sans-serif; position:relative; }
...
.system-metrics { --edge:#e2e8f0; margin:14px 16px 0; display:grid; grid-template-columns:repeat(4,1fr); gap:12px; }
.metric-card { min-height:78px; border:1px solid var(--edge); background:#ffffff; padding:12px 14px; display:flex; align-items:center; gap:12px; position:relative; overflow:hidden; border-radius:12px; box-shadow:0 1px 3px rgba(0,0,0,.08); }
...
.metric-icon { width:34px; height:34px; display:grid; place-items:center; color:var(--tone); background:color-mix(in srgb,var(--tone) 10%,transparent); border:1px solid color-mix(in srgb,var(--tone) 22%,transparent); font-size:17px; }
