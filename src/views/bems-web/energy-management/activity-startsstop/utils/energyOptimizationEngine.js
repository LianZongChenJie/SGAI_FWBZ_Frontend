...
/**
 * 从点位快照重算启停求解与运行参数推荐，并把结果写回点位表。
 * 「分析与启停求解」和「活动排期启停求解」两页共用，保证结论一致。
 * raw: key => 当前值；write: (key, value) => 写回。
 */
export function recalculateExhibitionRecommendations(raw, write) {
  ...
  write('optimization.recommendation.plantStartTime', result.plantStartTime)
  write('optimization.recommendation.plantStopTime', result.plantStopTime)
  write('optimization.recommendation.preCoolingMinutes', result.preCoolingMinutes)
  write('optimization.recommendation.chwSupplyTemp', result.controls.chwSupplyTemp)
  write('optimization.recommendation.chwReturnTempTarget', result.controls.chwReturnTempTarget)
  write('optimization.recommendation.chwPumpFrequency', result.controls.chwPumpFrequency)
  write('optimization.recommendation.cwPumpFrequency', result.controls.cwPumpFrequency)
  write('optimization.recommendation.towerFrequency', result.controls.towerFrequency)
  write('optimization.recommendation.chillerCount', result.controls.chillerCount)
  write('optimization.recommendation.expectedSavingRate', result.controls.expectedSavingRate)
  write('optimization.recommendation.conclusion', result.conclusion)
  Object.entries(result.schedule).forEach(([key, value]) => { write(`optimization.schedule.plant.${key}`, value) })
  result.hallSchedules.forEach(hall => {
    Object.entries(hall).forEach(([key, value]) => { write(`optimization.schedule.hall${hall.no}.${key}`, value) })
  })
  return result
}
