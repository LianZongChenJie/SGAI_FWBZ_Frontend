      ...
      <section class="schedule-grid">
        <div class="schedule-main">
          ...
          <article class="expert-card linkage-card">
            <header>
              <div><span class="section-no">02</span><div><h2>活动与设备启停联动</h2><p>预冷段（能源站启动 → 开馆）· 供冷段（开馆 → 建议停机）· 余冷滑行（停机 → 闭馆）</p></div></div>
          ...
          <article class="expert-card equipment-card">
            <header>
              <div><span class="section-no">03</span><div><h2>设备启停时序</h2><p>能源站与分馆泵组、冷水机组、冷却塔的错峰启动方案</p></div></div>
            </header>
...
function renderCharts() {
  if (!energyChartRef.value || !linkageChartRef.value) return
...
function showChartPlaceholder(chartInstance, message) {
  chartInstance.setOption({
  title: { text: message, left: 'center', top: 'center', textStyle: { color: '#94a3b8', fontSize: 16, fontWeight: 'normal' } }, xAxis: { show: false }, yAxis: { show: false }, series: []
  })
}
...
function renderLinkageChart() {
  ...
  const precoolData = rows.map(row => !row.legacy && row.openMin - row.startMin > 0
    ? { value: row.openMin - row.startMin, label: { show: true, position: 'insideLeft', formatter: row.start, color: '#ffffff', fontSize: 13 } }
    : 0)
  const runData = rows.map(row => row.legacy
    ? { value: Math.max(0, row.stopMin - row.startMin), itemStyle: { color: '#94a3b8' }, label: { show: true, position: 'insideLeft', formatter: '固定时刻 08:00–18:30 · 冷机外部管理', color: '#ffffff', fontSize: 13 } }
    : { value: Math.max(0, row.stopMin - row.openMin), label: { show: true, position: 'insideLeft', formatter: `开馆 ${row.open}`, color: '#ffffff', fontSize: 13 } })
  const coastData = rows.map(row => !row.legacy && row.closeMin - row.stopMin > 0
    ? { value: row.closeMin - row.stopMin, label: { show: true, position: 'right', formatter: `${row.stop} 停 → ${row.close} 闭`, color: '#1e293b', fontSize: 13 } }
    : 0)
  ...
  } }, xAxis: { type: 'value', min: minAxis, max: maxAxis, interval: 60, axisLabel: { color: '#94a3b8', fontSize: 14, formatter: value => `${String(Math.floor(value / 60)).padStart(2, '0')}:${String(value % 60).padStart(2, '0')}` }, splitLine: { lineStyle: { color: 'rgba(148,163,184,.15)' } } }, yAxis: { type: 'category', data: rows.map(row => row.legacy ? '1号馆·旧馆' : `${row.no}号馆`), axisLine: { lineStyle: { color: '#e2e8f0' } }, axisTick: { show: false }, axisLabel: { color: '#64748b', fontSize: 14 } }, series: [
    { name: '占位', type: 'bar', stack: 'hall', barWidth: 20, silent: true, legendHoverLink: false, itemStyle: { color: 'transparent' }, data: bases, tooltip: { show: false } },
...
.header-state{display:flex;align-items:center;gap:13px;font-size:14px;color:#94a3b8}
...
.kpi-grid span{font-size:16px;color:#94a3b8}
.kpi-grid strong{display:block;margin-top:4px;font-size:26px;color:#1e293b;font-variant-numeric:tabular-nums}
.kpi-grid strong small{font-size:20px;color:#94a3b8;margin-left:4px}
.kpi-grid em{display:block;margin-top:2px;font-size:13px;color:var(--tone);font-style:normal}
...
.schedule-grid{display:grid;grid-template-columns:minmax(0,1.5fr) minmax(330px,.8fr);gap:10px}
...
.expert-card>header>div:first-child{display:flex;align-items:center;justify-content:center}
.section-no{width:23px;height:23px;margin-right:9px;display:grid;place-items:center;border:1px solid #bae6fd;color:#0284c7;font-size:13px;border-radius:6px;background:#f0f9ff;flex-shrink:0}
.expert-card>header>div>div{display:flex;align-items:center;gap:8px}
.expert-card h2{font-size:20px;color:#2d3748;margin:0;line-height:1;display:flex;align-items:center}
.expert-card header p{margin:0;font-size:13px;color:#94a3b8;white-space:nowrap;display:flex;align-items:center}
.energy-badges{display:flex;gap:7px}
.energy-badges span{font-size:14px;color:#64748b;padding:5px 7px;border:1px solid var(--edge);border-radius:6px;background:#f8fafc}
.energy-badges b{color:#0284c7}
.energy-card{display:flex;flex-direction:column}.energy-chart{flex:1;min-height:200px;width:100%}
.chart-legend{height:39px;display:flex;align-items:center;justify-content:center;gap:18px;color:#64748b;font-size:16px;flex-wrap:wrap}
...
.influence{height:25px;padding:0 10px;display:grid;grid-template-columns:110px 1fr 28px;align-items:center;gap:6px;font-size:13px;color:#64748b}
...
.diff-list strong{color:#1e293b;font-size:20px;font-variant-numeric:tabular-nums}
...
.diff-list>div:last-child strong{color:#94a3b8;font-size:14px}
.schedule-tags{display:flex;align-items:center;gap:6px}
.schedule-tags span,.schedule-tags b{padding:4px 7px;border:1px solid rgba(14,165,233,.3);font-size:13px;font-weight:500;color:#0284c7;border-radius:4px;background:rgba(14,165,233,.05)}
...
.timeline-node b{font-size:22px;color:#1e293b}
.timeline-node span{margin-top:2px;font-size:20px;color:#475569}
.timeline-node small{font-size:12px;color:#94a3b8}
...
.schedule-table td strong,.schedule-table td b{display:block;color:#334155;font-size:16px}
.schedule-table td span{display:block;margin-top:2px;line-height:1.45;color:#94a3b8}
.schedule-table .time-cell strong{font-size:20px;color:#16a34a}
...
.optimization-toast{position:absolute;z-index:40;left:50%;bottom:34px;transform:translateX(-50%);padding:9px 16px;border:1px solid var(--edge);background:#ffffff;color:#1e293b;font-size:15px;border-radius:8px;box-shadow:0 12px 28px rgba(0,0,0,.12)}
