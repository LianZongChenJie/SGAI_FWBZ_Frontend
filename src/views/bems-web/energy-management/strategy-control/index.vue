  ...
  <div class="optimization-page">
      ...
      <section class="decision-grid control-view-grid">
        <article class="expert-card decision-card">
          ...
          <div class="expert-conclusion"
            ...
            ><span
              >预计节能 <b>{{ format('optimization.recommendation.expectedSavingRate', 1) }}%</b></span
            ></div
          >
          <div class="decision-table-wrap">
            <table class="decision-table">
              <thead
                ><tr
                  ><th>采用</th><th>对象 / 决策项</th><th>执行方式</th><th>当前</th><th>优化推荐</th><th>安全边界</th><th>推荐依据</th
                  ><th>点位编码</th></tr
                ></thead
              >
              <tbody>
                <tr v-for="item in decisions" :key="item.pointKey" :class="{ muted: !selectedCommands[item.pointKey] }">
                  <td
                    ><button
                      class="check"
                      :class="{ selected: selectedCommands[item.pointKey] }"
                      :aria-label="`${item.name}是否采用`"
                      @click="selectedCommands[item.pointKey] = !selectedCommands[item.pointKey]"
                      >✓</button
                    ></td
                  >
                  <td
                  ...
                  >
                  <td
                    ><span class="capability-badge" :class="item.capability">{{ capabilityMeta[item.capability].label }}</span></td
                  >
                  <td>{{ item.current }}</td>
        ...
        <aside class="expert-card control-card">
          ...
          <div class="guard-grid">
            ...
            <span
              ><i :class="fallbackReady ? 'ok' : 'warn'"></i>厂商基线 <b>{{ fallbackReady ? '可回退' : '不可用' }}</b></span
            >
          </div>
          <div class="capability-counts">
            <span class="direct"
              >直控 <b>{{ capabilityCounts.direct }}</b></span
            >
            <span class="manual"
              >人工确认 <b>{{ capabilityCounts.manual }}</b></span
            >
            <span class="advisory"
              >仅建议 <b>{{ capabilityCounts.advisory }}</b></span
            >
            <span class="unavailable"
              >不可控 <b>{{ capabilityCounts.unavailable }}</b></span
            >
          </div>
          <div class="dispatch-summary">
            <span
              >本拍已选 <b>{{ selectedCount }}</b> 条 / 可直控 <b>{{ directSelectedCount }}</b> 条</span
            >
            <span
              >数据完备度 <b>{{ format('optimization.control.dataReadiness', 0) }}%</b></span
            >
          </div>
          <div class="control-actions">
            <button
              class="dispatch-button"
              :disabled="mode !== 'closed' || actioning || directSelectedCount === 0"
              @click="openConfirm('dispatch')"
              >{{ mode === 'shadow' ? '影子模式 · 不下发' : mode === 'frozen' ? '系统已冻结' : `下发 ${directSelectedCount} 条直控策略` }}</button
            >
            <button class="rollback-button" :disabled="!fallbackReady || actioning" @click="openConfirm('rollback')">回退厂商基线</button>
          </div>
          <button class="manual-order-button" :disabled="manualSelectedCount === 0 || actioning" @click="createManualActions"
            >生成人工执行清单（{{ manualSelectedCount }} 条）</button
          >
          <div class="execution-monitor">
            <div
              ><span>执行进度</span><b>{{ format('optimization.control.executionProgress', 0) }}%</b></div
            >
    ...
    <div v-if="confirmAction" class="confirm-mask" @click.self="confirmAction = ''">
      <section class="confirm-dialog">
        <header
          ><span>{{ confirmAction === 'dispatch' ? 'CONTROL DISPATCH' : 'SAFE ROLLBACK' }}</span
          ><h2>{{ confirmAction === 'dispatch' ? '确认下发直控策略' : '确认回退厂商基线' }}</h2></header
        >
        <p v-if="confirmAction === 'dispatch'"
          >只提交
          {{ directSelectedCount }} 条“接口直控”策略。人工确认和仅建议项不会进入控制网关；服务端还会按当前能力矩阵二次拦截并执行安全边界钳位。</p
        >
        <p v-else>优化层将立即冻结，停止新策略下发，并向控制网关广播“厂商基线接管”命令。</p>
        <div
          ><button @click="confirmAction = ''">取消</button
          ><button :class="confirmAction === 'rollback' ? 'danger' : 'primary'" @click="executeConfirmed">{{
            actioning ? '处理中…' : '确认执行'
          }}</button></div
        >
      </section>
      ...
      <section class="capability-dialog">
        ...
        <div class="capability-list">
          <label v-for="item in decisions" :key="item.capabilityKey">
            <span
              ><b>{{ item.system }} · {{ item.name }}</b
              ><code>{{ item.capabilityKey }}</code></span
            >
...
<script setup>
  ...
  const flowStage = 4;
  const flowSteps = [
    { title: '采集', note: '站房 · 气象 · 会展' },
    { title: '预测', note: 'P50 / P90 冷负荷' },
    { title: '决策', note: '约束寻优与结论' },
    { title: '下发', note: '网关写点与确认' },
    { title: '验证', note: '执行监控与回退' },
  ];
  ...
  const decisionDefinitions = [
    {
      system: '能源站',
      name: '启动时间',
      capabilityKey: 'optimization.capability.plantStartTime',
      pointKey: 'optimization.recommendation.plantStartTime',
      current: '08:00',
      boundary: '最早 09:00 开馆',
      basis: '2号馆历史降温 2.7℃/h',
    },
    {
      system: '能源站',
      name: '停止时间',
      capabilityKey: 'optimization.capability.plantStopTime',
      pointKey: 'optimization.recommendation.plantStopTime',
      current: '20:30',
      boundary: '最晚 19:00 闭馆',
      basis: '3号馆余冷滑行 50 min',
    },
    {
      system: '冷冻站',
      name: '冷冻供水温度',
      capabilityKey: 'optimization.capability.chwSupplyTemp',
      pointKey: 'optimization.recommendation.chwSupplyTemp',
      current: '7.0 ℃',
      boundary: '5 ~ 9 ℃',
      basis: '峰段降供温 0.4℃',
    },
    {
      system: '冷冻站',
      name: '冷冻回水温度目标',
      capabilityKey: 'optimization.capability.chwReturnTempTarget',
      pointKey: 'optimization.recommendation.chwReturnTempTarget',
      current: '15.0 ℃',
      boundary: '10 ~ 16 ℃',
      basis: '大温差策略',
    },
    {
      system: '水泵',
      name: '冷冻泵频率',
      capabilityKey: 'optimization.capability.chwPumpFrequency',
      pointKey: 'optimization.recommendation.chwPumpFrequency',
      current: '42 Hz',
      boundary: '30 ~ 50 Hz',
      basis: '满足最小压差',
    },
    {
      system: '水泵',
      name: '冷却泵频率',
      capabilityKey: 'optimization.capability.cwPumpFrequency',
      pointKey: 'optimization.recommendation.cwPumpFrequency',
      current: '43 Hz',
      boundary: '30 ~ 50 Hz',
      basis: '逼近冷却塔逼近度',
    },
    {
      system: '冷却塔',
      name: '冷却塔频率',
      capabilityKey: 'optimization.capability.towerFrequency',
      pointKey: 'optimization.recommendation.towerFrequency',
      current: '38 Hz',
      boundary: '30 ~ 50 Hz',
      basis: '湿球温度修正',
    },
    {
      system: '冷水机组',
      name: '运行台数',
      capabilityKey: 'optimization.capability.chillerCount',
      pointKey: 'optimization.recommendation.chillerCount',
      current: '2 台',
      boundary: '1 ~ 3 台',
      basis: '负荷爬坡速率',
    },
    {
      system: '能源站',
      name: '预冷时长',
      capabilityKey: 'optimization.capability.preCoolingMinutes',
      pointKey: 'optimization.recommendation.preCoolingMinutes',
      current: '90 min',
      boundary: '20 ~ 180 min',
      basis: '降温梯度自学习',
    },
  ];
  decisionDefinitions.forEach((item) => {
    selectedCommands[item.pointKey] = true;
  });
  const decisions = computed(() =>
    decisionDefinitions.map((item) => ({
      ...item,
  ...
  const directSelectedCount = computed(
    () => decisions.value.filter((item) => selectedCommands[item.pointKey] && item.capability === 'direct').length
  );
  const manualSelectedCount = computed(
    () => decisions.value.filter((item) => selectedCommands[item.pointKey] && (item.capability === 'manual' || item.capability === 'advisory')).length
  );
  const capabilityCounts = computed(() =>
    decisions.value.reduce(
      (counts, item) => {
        counts[item.capability] += 1;
        return counts;
      },
      { direct: 0, manual: 0, advisory: 0, unavailable: 0 }
    )
  );
  ...
  const countdownText = computed(() => {
    const seconds = Math.max(0, numberValue('optimization.control.nextCycleSeconds'));
  ...
  function normalizePoints(payload) {
    ...
    return Object.entries(source).map(([key, value]) =>
      value && typeof value === 'object' && !Array.isArray(value) && 'value' in value ? { key, ...value } : { key, value }
    );
  }
  function applyPoints(payload) {
    ...
    const hasAlgorithmInput = incoming.some(
      (item) =>
        item.key?.startsWith('optimization.weather.') ||
        item.key?.startsWith('optimization.tariff.') ||
        item.key?.startsWith('optimization.event.') ||
        item.key?.startsWith('optimization.terminal.') ||
        item.key === 'optimization.forecast.nextHourP90'
    );
    const hasExternalRecommendation = incoming.some(
      (item) => item.key?.startsWith('optimization.recommendation.') || item.key?.startsWith('optimization.schedule.')
    );
  ...
  function recalculateRecommendations() {
    const result = optimizeExhibitionPlant({
      weather: {
        dryBulb: raw('optimization.weather.dryBulb'),
        wetBulb: raw('optimization.weather.wetBulb'),
        solarRadiation: raw('optimization.weather.solarRadiation'),
      },
      ...
      loadP90: raw('optimization.forecast.nextHourP90'),
      designCapacity: 9000,
      plantPower: raw('optimization.performance.currentPower'),
    });
    ...
    Object.entries(result.schedule).forEach(([key, value]) => {
      values[`optimization.schedule.plant.${key}`] = value;
    });
    result.hallSchedules.forEach((hall) => {
      Object.entries(hall).forEach(([key, value]) => {
        values[`optimization.schedule.hall${hall.no}.${key}`] = value;
      });
    });
  }
  ...
  function onOptimizationValues(event) {
    applyPoints(event.detail);
  }
  async function loadSnapshot() {
    try {
      const response = await fetch('/api/energy-optimization/points');
  ...
  async function setMode(nextMode) {
    ...
    if (nextMode === 'shadow') {
      values['optimization.control.dispatchState'] = '影子验证';
    } else if (nextMode === 'frozen') {
      values['optimization.control.dispatchState'] = '厂商接管';
    } else {
      values['optimization.control.dispatchState'] = '待下发';
    }
    try {
      const response = await fetch('/api/energy-optimization/mode', {
        method: 'POST',
  ...
  async function setCapability(key, capability) {
    ...
    history.value.unshift({
      ...
      title: `能力矩阵更新 · ${meta ? meta.name : key}`,
      state: label,
      tone: 'neutral',
    });
    try {
      const response = await fetch('/api/energy-optimization/capability', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pointKey: key, capability }),
      });
      if (response.ok) applyPoints(await response.json());
    } catch {}
  ...
  async function createManualActions() {
    ...
    try {
      const commands = decisions.value
        .filter((item) => selectedCommands[item.pointKey] && (item.capability === 'manual' || item.capability === 'advisory'))
        .map((item) => ({ pointKey: item.pointKey, name: item.name, target: numberValue(item.pointKey), capability: item.capability }));
      const response = await fetch('/api/energy-optimization/manual-actions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      history.value.unshift({
        id: `H-${Date.now()}`,
        time: new Date().toTimeString().slice(0, 5),
        title: `人工清单 ${commands.length} 条待确认`,
        state: '待确认',
        tone: 'warn',
      });
  ...
  async function executeConfirmed() {
    ...
    try {
      ...
      if (action === 'dispatch') {
        const commands = decisions.value
          .filter((item) => selectedCommands[item.pointKey] && item.capability === 'direct')
          .map((item) => ({ pointKey: item.pointKey, name: item.name, target: numberValue(item.pointKey) }));
        const response = await fetch('/api/energy-optimization/dispatch', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ commands }),
        });
        ...
        history.value.unshift({
          id: `H-${Date.now()}`,
          time: new Date().toTimeString().slice(0, 5),
          title: `直控下发 ${commands.length} 条`,
          state: '成功',
          tone: 'ok',
        });
        showToast(`已下发 ${commands.length} 条直控策略`);
      } else {
        const response = await fetch('/api/energy-optimization/rollback', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ reason: '人工确认' }),
        });
        if (response.ok) applyPoints(await response.json());
  ...
  async function onOptimizationCommand(event) {
    ...
    if (action === 'dispatch') {
      ...
      await executeConfirmed();
    } else if (action === 'manual' || action === 'manual-actions') {
      await createManualActions();
    }
  }
  ...
  const optimizationApi = {
    ...
    setCapability,
    dispatch: (commands) =>
      fetch('/api/energy-optimization/dispatch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ commands }),
      }),
    createManualActions: (commands) =>
      fetch('/api/energy-optimization/manual-actions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ commands }),
      }),
    rollback: (reason) =>
      fetch('/api/energy-optimization/rollback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  onMounted(async () => {
    ...
    countdownTimer = setInterval(() => {
      const key = 'optimization.control.nextCycleSeconds';
      values[key] = numberValue(key) > 0 ? numberValue(key) - 1 : numberValue('optimization.control.cycleMinutes') * 60;
    }, 1000);
  ...
  onUnmounted(() => {
    ...
    clearTimeout(toastTimer);
    window.removeEventListener('energy-optimization-values', onOptimizationValues);
    window.removeEventListener('energy-optimization-command', onOptimizationCommand);
...
<style scoped>
  ...
  .logic-flow > div.active {
    opacity: 1;
  }
  .logic-flow > div.active b {
    color: #ffffff;
    background: var(--green);
  ...
  .expert-card > header > div:first-child {
    display: flex;
    align-items: center;
  ...
  .decision-table td span {
    display: block;
    margin-top: 2px;
    font-size: 13px;
    color: #94a3b8;
  }
  .decision-table td.recommended {
    color: #16a34a;
    font-weight: 700;
  }
  .decision-table code {
  ...
  .capability-counts .direct b {
    color: #16a34a;
  }
  .capability-counts .manual b {
    color: #0284c7;
  }
  .capability-counts .advisory b {
    color: #d97706;
  }
  .capability-counts .unavailable b {
    color: #94a3b8;
  }
  .dispatch-summary {
    display: flex;
  ...
  .execution-monitor > div {
    display: flex;
    justify-content: space-between;
    font-size: 13px;
    color: #64748b;
  }
  .execution-monitor > div b {
    color: #16a34a;
  }
  .execution-monitor > i {
    display: block;
  ...
  .history-list b.danger {
    color: #dc2626;
  }
  .history-list b.neutral {
    color: #94a3b8;
  }
  .capability-badge {
    display: inline-block;
  ...
  .capability-badge.direct {
    color: #16a34a;
    border-color: rgba(34, 197, 94, 0.35);
    background: rgba(34, 197, 94, 0.07);
  }
  .capability-badge.manual {
    color: #0284c7;
    border-color: rgba(14, 165, 233, 0.35);
    background: rgba(14, 165, 233, 0.07);
  }
  .capability-badge.advisory {
    color: #d97706;
  ...
  .capability-dialog header span {
    font-size: 13px;
    letter-spacing: 2px;
    color: #0284c7;
  }
  .capability-dialog header h2 {
    margin-top: 4px;
    font-size: 20px;
    color: #1e293b;
  }
  .capability-dialog header p {
    margin-top: 3px;
    font-size: 14px;
    color: #94a3b8;
  }
  .capability-dialog header > button {
    width: 28px;
    height: 28px;
    border: 1px solid #e2e8f0;
    background: #f8fafc;
  ...
  .capability-list label span {
    display: flex;
    flex-direction: column;
  }
  .capability-list label b {
    font-size: 16px;
    color: #334155;
  }
  .capability-list label code {
    margin-top: 3px;
    font-size: 12px;
    color: #94a3b8;
  }
  .capability-list select {
    height: 29px;
    padding: 0 8px;
    border: 1px solid #e2e8f0;
    background: #ffffff;
  ...
  .capability-dialog footer span {
    font-size: 13px;
    color: #94a3b8;
  }
  .capability-dialog footer button {
    padding: 6px 15px;
    border: 1px solid #0ea5e9;
    background: #0284c7;
    color: white;
  ...
  /* .control-view-grid .decision-table-wrap {
    max-height: 430px;
  } */
  .control-view-grid .control-card {
    min-height: 500px;
  }
