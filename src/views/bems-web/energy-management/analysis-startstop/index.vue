...
function renderCharts() {
  ...
  loadChart.setOption({ animationDuration: 500, grid: { left: 60, right: 36, top: 46, bottom: 40 }, tooltip: { trigger: 'axis', backgroundColor: '#ffffff', borderColor: '#e2e8f0', textStyle: { color: '#2d3748', fontSize:16 } }, xAxis: { type: 'category', data: chartHours, boundaryGap: false, axisLine: { lineStyle: { color: '#e2e8f0' } }, axisLabel: { color: '#94a3b8', fontSize: 16, interval: 1 } }, yAxis: { type: 'value', name: 'kW', nameTextStyle: { color: '#94a3b8', fontSize: 16 }, axisLabel: { color: '#94a3b8', fontSize: 16 }, splitLine: { lineStyle: { color: 'rgba(148,163,184,.25)' } } }, series: [
...
.kpi-grid article::after {
  ...
  width: 65px;
  height: 65px;
  border: 12px solid color-mix(in srgb, var(--tone) 8%, transparent);
  border-radius: 50%;
}
.kpi-grid .blue { --tone: #3b82f6; }
.kpi-grid .cyan { --tone: #0ea5e9; }
.kpi-grid .violet { --tone: #8b5cf6; }
.kpi-grid .green { --tone: #22c55e; }
.kpi-grid .amber { --tone: #f59e0b; }
.kpi-grid .red { --tone: #ef4444; }
.kpi-grid .slate { --tone: #8296a3; }
.kpi-grid span {
  font-size: 16px;
  color: #94a3b8;
}
...
.hall-list > div {
  display: grid;
  grid-template-columns: 40px 1fr 62px 51px;
  align-items: center;
  gap: 5px;
  padding: 5px 0;
  border-bottom: 1px solid #f0f0f0;
...
.plant-timeline {
  ...
  position: relative;
  display: grid;
  grid-template-columns: repeat(4, minmax(110px, 1fr)) 260px;
  align-items: center;
  gap: 10px;
}
.timeline-line {
  ...
  top: 48px;
  height: 2px;
  background: linear-gradient(90deg, #0ea5e9, #22c55e 42%, #f59e0b 73%, #ef4444);
}
.timeline-line i {
  display: block;
  width: 72%;
...
.plant-timeline aside span {
  font-size: 13px;
  color: #64748b;
}
.plant-timeline aside b {
  float: right;
  color: #16a34a;
  font-size: 15px;
}
...
.schedule-table .time-cell strong {
  font-size: 20px;
  color: #16a34a;
}
...
.schedule-table td:last-child i {
  display: inline-block;
  margin-top: 3px;
  padding: 2px 4px;
  font-style: normal;
