...
function renderCharts() {
  ...
  loadChart.setOption({ animationDuration: 500, grid: { left: 58, right: 22, top: 28, bottom: 40 }, tooltip: { trigger: 'axis', backgroundColor: '#ffffff', borderColor: '#e2e8f0', textStyle: { color: '#2d3748', fontSize:16 } }, xAxis: { type: 'category', data: chartHours, boundaryGap: false, axisLine: { lineStyle: { color: '#e2e8f0' } }, axisLabel: { color: '#94a3b8', fontSize: 16, interval: 1 } }, yAxis: { type: 'value', name: 'kW', nameTextStyle: { color: '#94a3b8', fontSize: 16 }, axisLabel: { color: '#94a3b8', fontSize: 16 }, splitLine: { lineStyle: { color: 'rgba(148,163,184,.25)' } } }, series: [
    ...
    { name: 'P50 预测', type: 'line', data: p50, smooth: true, symbol: 'none', lineStyle: { width: 2.5, color: '#38bde1' }, markArea: { silent: true, data: [[{ xAxis: '09', itemStyle: { color: 'rgba(34,197,94,.12)' }, label: { show: true, formatter: '会展开馆', color: '#16a34a', fontSize: 14 } }, { xAxis: '19' }], [{ xAxis: '10', itemStyle: { color: 'rgba(245,158,11,.12)' }, label: { show: true, formatter: '峰价', color: '#d97706', fontSize: 14 } }, { xAxis: '15' }], [{ xAxis: '18', itemStyle: { color: 'rgba(245,158,11,.12)' } }, { xAxis: '21' }]] } },
...
<style scoped>
.optimization-page{--ink:#2d3748;--muted:#64748b;--edge:#e2e8f0;--panel-bg:#ffffff;--cyan:#0ea5e9;--green:#22c55e;--blue:#3b82f6;--amber:#f59e0b;--red:#ef4444;height:100%;min-height:0;display:flex;flex-direction:column;color:var(--ink);background:#f5f7fa;font-family:"PingFang SC","Microsoft YaHei",sans-serif;position:relative}.optimization-header{height:54px;flex:none;padding:0 20px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--edge);background:#ffffff}.optimization-header .eyebrow{color:#94a3b8;font-size:15px;font-weight:700;letter-spacing:2px}.optimization-header h1{font-size:25px;margin:2px 0 1px;color:#1e293b}.optimization-header p{font-size:15px;color:#94a3b8}.header-state{display:flex;align-items:center;gap:13px;font-size:14px;color:#94a3b8}.header-state span{padding:6px 9px;border:1px solid rgba(34,197,94,.3);color:#16a34a;border-radius:6px}.header-state span i{display:inline-block;width:6px;height:6px;margin-right:6px;border-radius:50%;background:var(--green);box-shadow:0 0 7px rgba(34,197,94,.4)}.header-state b{font-weight:500;color:#64748b}.header-state em{font-style:normal}.logic-flow{height:58px;flex:none;padding:8px 20px;display:grid;grid-template-columns:repeat(5,1fr);border-bottom:1px solid var(--edge);background:#ffffff}.logic-flow>div{display:flex;align-items:center;position:relative;opacity:.5}.logic-flow b{width:25px;height:25px;display:grid;place-items:center;border:1px solid #cbd5e1;border-radius:50%;font-size:15px;color:#94a3b8}.logic-flow span{display:flex;flex-direction:column;margin-left:8px}.logic-flow strong{font-size:16px;color:#334155}.logic-flow small{font-size:13px;color:#94a3b8;margin-top:2px}.logic-flow i{position:absolute;right:10px;color:#cbd5e1;font-style:normal}.logic-flow>div.active{opacity:1}.logic-flow>div.active b{color:#ffffff;background:var(--green);border-color:var(--green);box-shadow:0 0 12px rgba(34,197,94,.25)}.optimization-scroll{flex:1;min-height:0;overflow:auto;padding:12px 0 20px;background-color:#f5f7fa;background-image:linear-gradient(rgba(148,163,184,.07) 1px,transparent 1px),linear-gradient(90deg,rgba(148,163,184,.07) 1px,transparent 1px);background-size:24px 24px;display:flex;flex-direction:column;align-items:center}.kpi-grid,.analysis-grid{width:100%}.kpi-grid{display:grid;grid-template-columns:repeat(6,1fr);gap:9px;margin-bottom:10px}.kpi-grid article{min-height:77px;padding:11px 12px;border:1px solid var(--edge);border-top:2px solid var(--tone);background:var(--panel-bg);border-radius:12px;box-shadow:0 1px 3px rgba(0,0,0,.08);position:relative;overflow:hidden}.kpi-grid article::after{content:"";position:absolute;right:-18px;bottom:-26px;width:65px;height:65px;border:12px solid color-mix(in srgb,var(--tone) 8%,transparent);border-radius:50%}.kpi-grid .blue{--tone:#3b82f6}.kpi-grid .cyan{--tone:#0ea5e9}.kpi-grid .violet{--tone:#8b5cf6}.kpi-grid .green{--tone:#22c55e}.kpi-grid .amber{--tone:#f59e0b}.kpi-grid .red{--tone:#ef4444}.kpi-grid span{font-size:16px;color:#94a3b8}.kpi-grid strong{display:block;margin-top:4px;font-size:26px;color:#1e293b;font-variant-numeric:tabular-nums}.kpi-grid strong small{font-size:20px;color:#94a3b8;margin-left:4px}.kpi-grid em{display:block;margin-top:2px;font-size:13px;color:var(--tone);font-style:normal}.expert-card{border:1px solid var(--edge);background:var(--panel-bg);border-radius:12px;box-shadow:0 1px 3px rgba(0,0,0,.08);overflow:hidden}.expert-card>header{height:48px;padding:0 13px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #f0f0f0}.expert-card>header>div:first-child{display:flex;align-items:center;justify-content:center}.section-no{width:23px;height:23px;margin-right:9px;display:grid;place-items:center;border:1px solid #bae6fd;color:#0284c7;font-size:13px;border-radius:6px;background:#f0f9ff;flex-shrink:0}.expert-card>header>div>div{display:flex;align-items:center;gap:8px}.expert-card h2{font-size:20px;color:#2d3748;margin:0;line-height:1;display:flex;align-items:center;}.expert-card header p{margin:0;font-size:13px;color:#94a3b8;white-space:nowrap;display:flex;align-items:center}.analysis-grid{display:grid;grid-template-columns:minmax(0,1.5fr) minmax(330px,.8fr);gap:10px}.forecast-card{min-height:386px;display:flex;flex-direction:column}.forecast-badges{display:flex;gap:7px}.forecast-badges span{font-size:16px;color:#64748b;padding:5px 7px;border:1px solid var(--edge);border-radius:6px;background:#f8fafc}.forecast-badges b{color:#0284c7}.load-chart{flex:1;min-height:200px}.chart-legend{height:39px;display:flex;align-items:center;justify-content:center;gap:18px;color:#64748b;font-size:16px}.chart-legend span{display:flex;align-items:center;gap:5px}.chart-legend i{display:inline-block}.chart-legend .band{width:15px;height:7px;border-radius:2px;background:rgba(14,165,233,.22)}.chart-legend i.line{flex:none;width:15px;height:3px;background:#0ea5e9}.chart-legend .p50{background:#0ea5e9}.chart-legend .dot{width:7px;height:7px;border-radius:50%}.chart-legend .actual{background:#f97316}.chart-legend .block{width:12px;height:7px;border-radius:2px}.chart-legend .event{background:rgba(34,197,94,.25)}.chart-legend .peak{background:rgba(245,158,11,.25)}.input-stack{display:grid;grid-template-columns:1fr;gap:10px}.quality{font-size:13px;padding:3px 6px;border:1px solid;border-radius:4px}.quality.good{color:#16a34a;border-color:rgba(34,197,94,.32);background:rgba(34,197,94,.06)}.weather-now{display:grid;grid-template-columns:repeat(4,1fr);gap:1px;background:#e2e8f0;border:1px solid var(--edge);border-radius:8px;overflow:hidden}.weather-now>div{padding:7px;background:#ffffff;text-align:center}.weather-now span{display:block;color:#94a3b8;font-size:13px}.weather-now strong{display:inline-block;margin-top:2px;font-size:20px;color:#2d3748}.weather-now small{font-size:12px;color:#94a3b8;margin-left:2px}.weather-chart{height:57px}.influence{height:25px;padding:0 10px;display:grid;grid-template-columns:110px 1fr 28px;align-items:center;gap:6px;font-size:13px;color:#64748b}.influence>b{height:3px;background:#e2e8f0;border-radius:2px;overflow:hidden}.influence>b i{display:block;height:100%;background:#0ea5e9}.influence>strong{color:#0284c7}.hall-list{padding:4px 10px 1px}.hall-list>div{display:grid;grid-template-columns:40px 1fr 62px 51px;align-items:center;gap:5px;padding:5px 0;border-bottom:1px solid #f0f0f0;font-size:13px}.hall-list>div>b{font-size:20px;color:#334155}.hall-list>div>span,.hall-list>div>em{color:#64748b;font-style:normal}.hall-list>div>small{padding:2px 3px;text-align:center;color:#d97706;background:rgba(245,158,11,.1);border-radius:4px}.hall-list p{grid-column:1/-1;margin-top:2px;color:#64748b}.hall-list p strong{color:#16a34a}.hall-list p i{float:right;color:#0284c7;font-style:normal}.event-influence>b i{background:#22c55e}.optimization-toast{position:absolute;z-index:40;left:50%;bottom:34px;transform:translateX(-50%);padding:9px 16px;border:1px solid var(--edge);background:#ffffff;color:#1e293b;font-size:15px;border-radius:8px;box-shadow:0 12px 28px rgba(0,0,0,.12)}.tariff-strip{height:25px;padding:0 10px;display:flex;align-items:center;gap:8px;border-top:1px solid #f0f0f0;border-bottom:1px solid #f0f0f0;font-size:13px;color:#64748b}.tariff-strip span{color:#d97706}.tariff-strip span i{display:inline-block;width:5px;height:5px;margin-right:5px;border-radius:50%;background:#f59e0b;box-shadow:0 0 7px rgba(245,158,11,.5)}.tariff-strip b{color:#b45309}.tariff-strip em{margin-left:auto;font-style:normal;color:#94a3b8}.schedule-card{margin-top:10px}.schedule-tags{display:flex;align-items:center;gap:6px}.schedule-tags span,.schedule-tags b{padding:4px 7px;border:1px solid rgba(14,165,233,.3);font-size:13px;font-weight:500;color:#0284c7;border-radius:4px;background:rgba(14,165,233,.05)}.schedule-tags .analysis-only{border-color:rgba(245,158,11,.32);color:#d97706;background:rgba(245,158,11,.06)}.plant-timeline{height:102px;margin:0 13px;position:relative;display:grid;grid-template-columns:repeat(4,minmax(110px,1fr)) 260px;align-items:center;gap:10px}.timeline-line{position:absolute;left:5%;right:285px;top:48px;height:2px;background:linear-gradient(90deg,#0ea5e9,#22c55e 42%,#f59e0b 73%,#ef4444)}.timeline-line i{display:block;width:72%;height:100%;background:rgba(0,0,0,.06)}.timeline-node{position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;text-align:center}.timeline-node::before{content:"";width:10px;height:10px;margin-bottom:5px;border:2px solid #ffffff;border-radius:50%;background:#0ea5e9;box-shadow:0 0 0 2px rgba(14,165,233,.25)}.timeline-node.stop::before{background:#f59e0b;box-shadow:0 0 0 2px rgba(245,158,11,.25)}.timeline-node.close::before{background:#ef4444;box-shadow:0 0 0 2px rgba(239,68,68,.23)}.timeline-node b{font-size:22px;color:#1e293b}.timeline-node span{margin-top:2px;font-size:20px;color:#475569}.timeline-node small{font-size:12px;color:#94a3b8}.plant-timeline aside{height:65px;padding:8px 10px;border-left:1px solid var(--edge);display:grid;align-content:center;gap:7px;background:#f8fafc;border-radius:0 8px 8px 0}.plant-timeline aside span{font-size:13px;color:#64748b}.plant-timeline aside b{float:right;color:#16a34a;font-size:15px}.schedule-table-wrap{overflow:auto;border-top:1px solid var(--edge)}.schedule-table{width:100%;border-collapse:collapse;font-size:13px}.schedule-table th{padding:7px 9px;text-align:left;color:#475569;background:#f8fafc;font-weight:500;white-space:nowrap}.schedule-table td{padding:8px 9px;border-top:1px solid #f0f0f0;color:#64748b;vertical-align:top}.schedule-table td strong,.schedule-table td b{display:block;color:#334155;font-size:20px}.schedule-table td span{display:block;margin-top:2px;line-height:1.45;color:#94a3b8}.schedule-table .time-cell strong{font-size:20px;color:#16a34a}.schedule-table .time-cell span{color:#94a3b8}.schedule-table td:last-child{min-width:170px}.schedule-table td:last-child i{display:inline-block;margin-top:3px;padding:2px 4px;font-style:normal;color:#0284c7;border:1px solid rgba(14,165,233,.25);border-radius:4px}.view-tabs{display:flex;border:1px solid var(--edge);border-radius:8px;background:#f8fafc;overflow:hidden}.view-tabs button{height:25px;padding:0 9px;border:0;border-right:1px solid var(--edge);background:transparent;color:#64748b;font-size:13px}.view-tabs button:last-child{border-right:0}.view-tabs button.active{color:#0284c7;background:rgba(14,165,233,.1);box-shadow:inset 0 -2px #0ea5e9}@media(max-width:1180px){.kpi-grid{grid-template-columns:repeat(3,1fr)}.analysis-grid{grid-template-columns:1fr}.input-stack{grid-template-columns:1fr 1fr}}@media(max-width:1180px){.plant-timeline{grid-template-columns:repeat(4,1fr)}.plant-timeline aside{display:none}.timeline-line{right:5%}}@media(prefers-reduced-motion:reduce){*{animation:none!important;transition:none!important}}
</style>
