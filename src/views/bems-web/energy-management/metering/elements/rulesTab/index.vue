<template>
  <div class="rules-tab">
    <div class="stat-cards">
      <StatCard label="计量项目总数" :value="statData.count" :change-text="statData.addCount" trend="" color="blue" :icon="ClipboardIcon" />
      <StatCard label="已配置公式" :value="statData.formulaCount" :change-text="statData.coverage" trend="" color="green" :icon="CheckSquareIcon" />
      <StatCard
        label="电表项目"
        ...
        :icon="ThunderIcon"
      />
      <StatCard label="水表项目" :value="statData.waterCount" :change-text="statData.waterPercentage" trend="" color="purple" :icon="WaterDropIcon" />
    </div>
...
<script setup lang="ts">
  ...
  // Tab 类型常量
  const typeTabs = [
    { key: 'specialty', label: '专业耗电数据' },
    { key: 'space', label: '空间耗电数据' },
    { key: 'run', label: '配电拓扑数据' },
    { key: 'space_water', label: '空间用水数据' },
    { key: 'specialty_water', label: '专业用水数据' },
    { key: 'water_data', label: '物理水表数据' },
  ];

  const activeType = ref(typeTabs[0].key);
  ...
  const handleTabChange = () => {
    // type 变化会自动触达内层 MeasureRule 的 watch 加载数据
  };
