...
/* 弹窗标题栏样式由下方非 scoped 的 .video-monitor-modal 统一控制（含标题与关闭按钮） */
...
/* 监控视频内容：暂时去掉放大裁边（近似 cover）处理，按 iframe 原始尺寸等比显示，
   不再裁切顶部信息条/上下留白；播放区宽高后续再调整。
   如需恢复铺满效果：放开下面 .video-player 的 transform，通过 --vm-zoom /
   --vm-origin-y 微调（放大后用容器裁掉上下留白，近似 cover）。 */
.video-box {
  /* --vm-zoom: 1.22; */
  /* --vm-origin-y: 40%; */
}
...
.video-box :deep(.video-player) {
  /* 去掉缩放：按原本大小展示（宽高后续再调整） */
  /* transform: scale(var(--vm-zoom)); */
  /* transform-origin: 50% var(--vm-origin-y); */
}
...
.theme-white .camera-label {
  ...
  /* 去掉缩放：按原本大小展示（宽高后续再调整） */
}.theme-white .video-box :deep(.video-player) {
  /* transform: scale(var(--vm-zoom)); */
  /* transform-origin: 50% var(--vm-origin-y); */
