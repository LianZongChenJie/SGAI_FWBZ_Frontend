...
const columns: BasicColumn[] = [
  ...
  {
    title: '等级编号',
    dataIndex: 'alarmLevelCode',
    key: 'alarmLevelCode',
    resizable: true,
    sorter: (a, b) => a.alarmLevelCode.localeCompare(b.alarmLevelCode), // 自定义排序函数
  ...
  },
  {
    ...
    key: 'alarmLevelName',
    resizable: true,
    sorter: (a, b) => a.alarmLevelName.localeCompare(b.alarmLevelName), // 自定义排序函数
    sortDirections: ['ascend', 'descend'],
  },
  {
    title: '创建人',
...
const getLinkageControlList = async (pageParams) => {
  ...
  let params = {
    pageNo: pageNo,
    pageSize: pageSize,
    alarmLevelCode: searchData.name ? searchData.name.split('*')[1] : undefined,
    alarmLevelName: searchData.name ? searchData.name.split('*')[1] : undefined,
