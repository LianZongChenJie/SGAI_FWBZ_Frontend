<template>
  <a-modal
    ...
    wrapClassName="scene-confirm-modal-bigscreen"
   :class="themeClass">
    <!-- 顶部提示条 -->
...
import { useScreenTheme } from '../../northAreaLightingSys/useScreenTheme';
...
const okText = ref('确定')
// 大屏版本：宽度放大一倍
const modalWidth = ref<string | number>('max(10rem, 1000px)')
 ...
 * width 可自定义弹框宽度（默认 10rem，兼容大屏 rem 布局；内容较长时建议传固定 px 值）；
...
function showModal(opts: {
  ...
  modalWidth.value = opts.width ?? 'max(10rem, 1000px)'
...
.modal-content-scroll {
  min-height: max(3.2rem, 240px);
  max-height: max(5.6rem, 520px);
  overflow-y: auto;
  position: relative;
  z-index: 1;
...
.modal-tip {
  ...
  align-items: center;
  gap: max(0.2rem, 20px);
  padding: max(0.2rem, 20px) max(0.28rem, 28px);
  margin-bottom: 0;
  background: rgba(0, 162, 232, 0.06);
  border-left: max(0.06rem, 6px) solid #00a2e8;
  border-radius: 0 max(0.08rem, 8px) max(0.08rem, 8px) 0;
  font-size: max(0.34rem, 30px);
  line-height: 1.6;
  color: #8a9ab0;
...
.modal-footer {
  ...
  gap: max(0.2rem, 20px);
  padding: max(0.32rem, 32px) max(0.48rem, 48px) max(0.24rem, 24px);
  margin: max(0.48rem, 48px) min(-0.48rem, -48px) min(-0.48rem, -48px);
  border-top: 1px dashed rgba(0, 212, 255, 0.25);
  background: rgba(6, 18, 36, 0.55);
  border-radius: 0 0 max(0.06rem, 6px) max(0.06rem, 6px);

  :deep(.ant-btn) {
    height: max(0.64rem, 64px);
    padding: 0 max(0.36rem, 36px);
...
/* ==================== scene-confirm-modal-bigscreen 弹框样式 ==================== */
<style lang="less">
.scene-confirm-modal-bigscreen {
  ...
  .ant-modal-header {
    ...
    padding: max(0.32rem, 32px) max(0.48rem, 48px) !important;
  ...
  .ant-modal-title {
    ...
    padding-left: max(0.24rem, 24px);
    text-shadow: 0 0 max(0.24rem, 24px) rgba(0, 212, 255, 0.4);
  }
  ...
  .ant-modal-close {
    top: max(0.32rem, 32px) !important;
    right: max(0.4rem, 40px) !important;

    .ant-modal-close-x {
      color: #7fa6d4 !important;
      font-size: max(0.36rem, 36px) !important;
  ...
  .ant-modal-body {
    min-height: max(3.6rem, 240px);
    padding: max(0.48rem, 48px) !important;
...
.modal-tip {
  ...
  border-left: max(0.06rem, 6px) solid #1890ff;
  border-radius: 0 max(0.08rem, 8px) max(0.08rem, 8px) 0;
  font-size: max(0.34rem, 30px);
  line-height: 1.6;
  color: #606266;
...
<style lang="less">
.theme-white .scene-confirm-modal-bigscreen {
