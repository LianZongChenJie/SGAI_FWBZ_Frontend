...
/** 展会总结报告列表（分页） */
export const getReportList = (params?: { pageNo?: number; pageSize?: number }) => defHttp.get({ url: Api.report, params });
...
/** 展会总结报告详情（queryById） */
export const getReportStatistics = (id: number) => defHttp.get({ url: Api.statistics, params: { id } });

/** 保存展会总结报告 */
export const saveReportStatistics = (data: ActiveMeetReport) => defHttp.post({ url: Api.saveStatistics, data });
...
/** 根据报告ID获取参展商列表 */
export const getExhibitorInfoList = (reportId: number) =>
  defHttp.get({ url: '/sgai-fwbz-dev/fwbz/exhibitorInfo/listByReportId', params: { reportId } });
