...
export function buildTrendOption(
  ...
  return {
    ...
    xAxis: {
      ...
      boundaryGap: false,
      data: xaxis,
      axisLabel: { color: TINK.muted, fontSize:14, interval: xaxis.length > 20 ? Math.ceil(xaxis.length / 10) : (xaxis.length > 10 ? 2 : 0) },
...
export function buildBarOption(
  ...
  return {
    ...
    xAxis: {
      type: 'category',
      data: categories,
      axisLabel: { color: TINK.muted, fontSize:14, interval: categories.length > 8 ? 2 : (categories.length > 5 ? 1 : 0) },
      axisLine: { lineStyle: { color: TINK.baseline } },
      axisTick: { show: false },
    },
    yAxis: {
