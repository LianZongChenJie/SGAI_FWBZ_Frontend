<template>
  <div class="ba-page ahu-2">
    <main>
      ...
      <div class="fan-rotor ahu-supply-rotor" :class="{ running: p('ba.ahu2.running') }"><i></i></div>
      <div class="valve-motion ahu-valve" :style="valveStyle(p('ba.ahu2.valve'))"><i></i></div>
      <div class="damper-motion ahu-fresh-damper" :style="valveStyle(p('ba.ahu2.freshDamper'))"><i v-for="x in 5" :key="`d${x}`"></i></div>
      ...
      <PointBadge class="pt fresh-air" label="新风温湿度" :value="`${n('ba.ahu2.freshTemp',1)}℃ · ${n('ba.ahu2.freshHumidity',1)}%RH`" />
      <PointBadge class="pt fresh-damper" label="新风阀反馈" :value="`${n('ba.ahu2.freshDamper',1)}%`" />
      <PointBadge class="pt filter-one" label="初效" :value="p('ba.ahu2.filter1Alarm')?'报警':'正常'" :alarm="p('ba.ahu2.filter1Alarm')" />
      <PointBadge class="pt filter-two" label="中效" :value="p('ba.ahu2.filter2Alarm')?'报警':'正常'" :alarm="p('ba.ahu2.filter2Alarm')" />
      <PointBadge class="pt valve-point" label="水阀反馈 / 调节" :value="`${n('ba.ahu2.valve',1)}%`" />
      <PointBadge class="pt water-point" label="供 / 回水温度" :value="`${n('ba.ahu2.supplyWaterTemp',1)} / ${n('ba.ahu2.returnWaterTemp',1)}℃`" />
      <PointBadge class="pt frost-point" label="防冻报警" :value="p('ba.ahu2.frostAlarm')?'报警':'正常'" :alarm="p('ba.ahu2.frostAlarm')" />
      <PointBadge class="pt humid-point" label="加湿控制" :value="p('ba.ahu2.humidifier')?'开':'关'" />
      <PointBadge class="pt fan-point" label="送风机状态 / 频率" :value="`${p('ba.ahu2.running')?'运行':'停止'} · ${n('ba.ahu2.fanFrequency',1)}Hz`" :alarm="p('ba.ahu2.fault')" />
      <PointBadge class="pt supply-air" label="送风温湿度" :value="`${n('ba.ahu2.supplyTemp',1)}℃ · ${n('ba.ahu2.supplyHumidity',1)}%RH`" />
...
.system-panel{position:absolute;right:16px;top:77px;bottom:14px;width:300px;padding-bottom:8px;border:1px solid #234b5e;background:#08202e;overflow-y:auto;overflow-x:hidden}
