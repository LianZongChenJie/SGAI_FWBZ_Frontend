  ...
  <div ref="viewportRef" class="schematic-viewport">
    <div class="plant-canvas" :style="canvasStyle">
      ...
      <div class="pipe-layer">
        <div v-for="(pipe, index) in pipes" :key="index" class="pipe" :class="[pipe.kind, pipe.dir, pipe.w > pipe.h ? 'horizontal' : 'vertical']" :style="pipeStyle(pipe)"><i v-if="pipe.dir"></i></div>
      </div>
      ...
      <section class="header-vessels">
        <div class="header-unit collector interactive-device is-normal" :class="motionClass('air.collector')" data-device-id="air.collector" @click="selectDevice('air.collector')"><div class="vertical-risers"><i v-for="n in 4" :key="n"><b></b></i></div><img src="/equipment/header-vessel-2_5d.png" alt="集水器"><span>集水器</span></div>
        <div class="header-unit distributor interactive-device is-normal" :class="motionClass('air.distributor')" data-device-id="air.distributor" @click="selectDevice('air.distributor')"><div class="vertical-risers"><i v-for="n in 4" :key="n"><b></b></i></div><img src="/equipment/header-vessel-2_5d.png" alt="分水器"><span>分水器</span></div>
...
const pipes = [
  ...
  { kind:'return',x:42,y:360,w:5,h:68,dir:'down' }, { kind:'return',x:42,y:423,w:27,h:5,dir:'right' },
  // 集水器右端出水上翻后进入水泵总管，绕开分水器
  { kind:'return',x:200,y:335,w:5,h:93,dir:'up' }, { kind:'return',x:200,y:335,w:350,h:5,dir:'right' },
  { kind:'return',x:545,y:335,w:5,h:248,dir:'down' },
  { kind:'return',x:545,y:446,w:238,h:5,dir:'right' }, { kind:'return',x:545,y:512,w:238,h:5,dir:'right' }, { kind:'return',x:545,y:578,w:238,h:5,dir:'right' },
  { kind:'return',x:778,y:385,w:5,h:198,dir:'up' }, { kind:'return',x:778,y:385,w:293,h:5,dir:'right' },
  { kind:'return',x:841,y:385,w:5,h:167,dir:'down' },
  { kind:'return',x:846,y:465,w:225,h:5,dir:'right' }, { kind:'return',x:846,y:547,w:225,h:5,dir:'right' },
  { kind:'return',x:1066,y:385,w:5,h:167,dir:'up' }, { kind:'return',x:1066,y:385,w:202,h:5,dir:'right' },
  { kind:'return',x:1263,y:38,w:5,h:352,dir:'up' }, { kind:'return',x:402,y:38,w:866,h:5,dir:'left' }, { kind:'return',x:402,y:188,w:866,h:5,dir:'left' },
  ...unitXs.flatMap(x => [{ kind:'return',x:x+8,y:38,w:5,h:42,dir:'down' }, { kind:'return',x:x+8,y:188,w:5,h:42,dir:'down' }]),
  // 冷冻供水：风冷机组 → 分水器 → 末端（蓝色）
  { kind:'supply',x:402,y:166,w:880,h:5,dir:'right' }, { kind:'supply',x:402,y:316,w:880,h:5,dir:'right' },
  ...unitXs.flatMap(x => [{ kind:'supply',x:x+120,y:135,w:5,h:36,dir:'down' }, { kind:'supply',x:x+120,y:285,w:5,h:36,dir:'down' }]),
  { kind:'supply',x:1277,y:166,w:5,h:190,dir:'down' }, { kind:'supply',x:343,y:351,w:939,h:5,dir:'left' },
  { kind:'supply',x:338,y:351,w:5,h:76,dir:'down' }, { kind:'supply',x:309,y:423,w:34,h:5,dir:'left' },
  // 集分水器支管与水处理旁通
  { kind:'return',x:98,y:350,w:5,h:70,dir:'down' }, { kind:'supply',x:258,y:350,w:5,h:70,dir:'down' },
  // 真空脱气机旁通：回水主管取水并回到同一回路
  { kind:'return',x:480,y:435,w:70,h:5,dir:'left' },
  { kind:'return',x:376,y:435,w:9,h:5,dir:'left' }, { kind:'return',x:376,y:435,w:5,h:31,dir:'down' },
  { kind:'return',x:376,y:462,w:174,h:5,dir:'right' },
  // 全程水处理器旁通：脱气机下游取水 → 设备 → 回水主管
  { kind:'return',x:477,y:530,w:73,h:5,dir:'left' },
  { kind:'return',x:387,y:556,w:9,h:5,dir:'left' }, { kind:'return',x:387,y:556,w:5,h:26,dir:'down' },
...
.header-vessels{position:absolute;left:62px;top:353px;width:292px;height:158px;z-index:4}.header-unit{position:absolute;top:58px;width:138px;height:84px}.header-unit.collector{left:0}.header-unit.distributor{right:0}.header-unit img{position:absolute;left:-4px;top:-9px;width:146px;height:96px;object-fit:contain;filter:drop-shadow(3px 4px 3px rgba(48,69,66,.22))}.header-unit>span{position:absolute;left:0;right:0;bottom:-7px;text-align:center;font-size:8px;font-weight:600;color:#3d5050;text-shadow:0 1px white}.vertical-risers{position:absolute;left:23px;top:-47px;width:91px;display:flex;justify-content:space-between}.vertical-risers>i{width:4px;height:56px;background:#51a82f;position:relative}.distributor .vertical-risers>i{background:#0879d0}.vertical-risers b{position:absolute;top:14px;left:-4px;width:11px;height:8px;background:#439849;border:2px solid #dde9df}.header-sensor{position:absolute;font-size:7px;color:#536466}.header-sensor b{padding:2px 4px;background:#caebe8;color:#337b78;font-weight:400}.collector-sensor{left:4px;top:12px}.distributor-sensor{right:0;top:31px}
