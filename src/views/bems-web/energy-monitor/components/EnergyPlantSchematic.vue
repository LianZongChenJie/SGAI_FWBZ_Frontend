  ...
  <div ref="viewportRef" class="schematic-viewport">
    <div class="plant-canvas" :style="canvasStyle">
      ...
      <div class="pipe-layer">
        <div v-for="(pipe, index) in pipes" :key="index" class="pipe" :class="[pipe.kind, pipe.dir, pipe.w > pipe.h ? 'horizontal' : 'vertical']" :style="pipeStyle(pipe)">
          <i v-if="pipe.dir"></i>
        </div>
      </div>
      ...
      <section class="header-vessels">
        <div class="header-unit collector interactive-device is-normal" :class="motionClass('water.collector')" data-device-id="water.collector" @click="selectDevice('water.collector')">
          <div class="vertical-risers"><i v-for="n in 4" :key="n"><b></b></i></div>
...
const pipes = [
  ...
  { kind:'chw-return',x:42,y:276,w:5,h:83,dir:'down' },
  { kind:'chw-return',x:42,y:354,w:24,h:5,dir:'right' },
  // 集水器右端出水先上翻再进入泵组总管，明确绕开分水器
  { kind:'chw-return',x:187,y:303,w:5,h:56,dir:'up' },
  { kind:'chw-return',x:187,y:303,w:195,h:5,dir:'right' },
  ...
  { kind:'chw-supply',x:315,y:240,w:247,h:5,dir:'left' },
  { kind:'chw-supply',x:310,y:240,w:5,h:116,dir:'down' },
  { kind:'chw-supply',x:288,y:354,w:27,h:5,dir:'left' },
  ...
  // 补水管
  { kind:'makeup',x:44,y:572,w:328,h:4,dir:'right' },
  { kind:'makeup',x:372,y:509,w:4,h:67,dir:'up' },
  ...
  // 补水泵出口接入冷冻回水总管
  { kind:'makeup',x:350,y:509,w:26,h:4,dir:'right' }
]
...
.header-vessels{position:absolute;left:58px;top:285px;width:285px;height:162px;z-index:3}.header-unit{position:absolute;top:50px;width:132px}.collector{left:0}.distributor{right:0}.vertical-risers{position:absolute;left:18px;top:-49px;width:94px;display:flex;justify-content:space-between}.vertical-risers>i{width:4px;height:51px;background:#62a74d;position:relative}.vertical-risers>i:nth-child(2){background:#d0bd00}.vertical-risers>i:nth-child(3){background:#d87319}.vertical-risers>i:nth-child(4){background:#0879d0}.vertical-risers b{position:absolute;top:12px;left:-4px;width:11px;height:8px;background:#439849;border:2px solid #dde9df}.vessel{height:42px;border:1px solid #8b9995;border-radius:21px;background:linear-gradient(#c7cfcb,#aeb9b5);box-shadow:inset 0 8px 10px rgba(255,255,255,.45);display:flex;align-items:center;justify-content:center;color:#3d4d4e;font-size:9px}.vessel>i{position:absolute;width:14px;height:30px;border:1px solid #8c9995;border-radius:50%}.vessel>i:first-child{left:8px}.vessel>i:last-child{right:8px}.vessel-feet{display:flex;justify-content:space-around}.vessel-feet i{width:5px;height:12px;background:#9ba7a3}.header-sensor{position:absolute;font-size:7px;color:#536466}.header-sensor b{padding:2px 4px;background:#caebe8;color:#337b78;font-weight:400}.supply-sensor{left:6px;top:7px}.return-sensor{right:0;top:26px}
...
.header-unit{top:42px;width:138px;height:84px}.header-vessel-image{position:absolute;left:-4px;top:-9px;width:146px;height:96px;object-fit:contain;filter:drop-shadow(3px 4px 3px rgba(48,69,66,.22))}.header-vessel-name{position:absolute;left:0;right:0;bottom:-7px;text-align:center;font-size:8px;font-weight:600;color:#3d5050;text-shadow:0 1px white}.header-unit .vertical-risers{left:23px;top:-39px;width:91px}.header-unit .vertical-risers>i{height:48px}.collector .vertical-risers>i{background:#51a82f}.distributor .vertical-risers>i{background:#0879d0}.collector .vertical-risers b,.distributor .vertical-risers b{background:#439849}.header-unit:hover .header-vessel-image{filter:drop-shadow(3px 4px 3px rgba(48,69,66,.2)) drop-shadow(0 0 4px rgba(42,143,111,.26))}
